...
var put_sound = preload("res://assets/sounds/effects/put.wav")
var take_sound = preload("res://assets/sounds/effects/take.wav")
...
func _on_area_2d_body_entered(body: Node2D) -> void:
	if body.is_in_group("pickupable"):
		...
		AudioManager.play_sound_at_position(put_sound, global_position)
...
func _item_taken(storage: Node2D, item: Node2D) -> void:
	if storage == self:
		...
		await get_tree().process_frame
		accept_item_animation.play("accept_item")
		AudioManager.play_sound_at_position(take_sound, global_position)
