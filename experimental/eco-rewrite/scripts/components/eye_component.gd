...
var sight_area_shape: CollisionShape2D
...
func handle_area_entered(area: Area2D) -> void:
	...
	if object == master_origin:
		return
	
	if object.is_in_group("plant"):
		nearby_plants.append(object)
		
func handle_area_exited(area: Area2D) -> void:
	var object = area.get_parent()
	if object.is_in_group("plant"):
		nearby_plants.erase(object)

func _ready() -> void:
	sight_area_shape = sight_area.get_node("CollisionShape2D")
