...
func _ready() -> void:
	var chunk: Vector2i = world_to_chunk(Vector2(0, 0))
...
func _setup_terrain_definitions() -> void:
	...
	terrain_definitions[0] = {
		...
		0: {"name": "dirt_path", "type": Cell.Type.GROUND, "nutrition": 3.0, "moisture": 0.0, "traversability": 1.0, "health": 10.0, "max_health": 10.0, "indestructible": true},
	}
...
func _sync_with_tilemap() -> void:
	...
	for tile_pos in used_cells:
		var chunk_pos = tile_to_chunk(Vector2(tile_pos))
		var local_pos = tile_to_local(Vector2(tile_pos))
...
func tile_to_chunk(tile_pos: Vector2) -> Vector2i:
	# convert global tile position to chunk position
	# chunk position is represented as units where each unit represents 64 cells in an 8x8 square
	
	return Vector2i(
		floori(tile_pos.x / Chunk.CHUNK_SIZE),
		floori(tile_pos.y / Chunk.CHUNK_SIZE),
	)	
	
func tile_to_local(tile_pos: Vector2i) -> Vector2i:
	# convert global tile position to a local position
	# the returned value should be paired with a chunk position in order to have meaning
	
	return Vector2i(
		posmod(tile_pos.x, Chunk.CHUNK_SIZE),
		posmod(tile_pos.y, Chunk.CHUNK_SIZE),
	)
...
func world_to_chunk(world_pos: Vector2) -> Vector2i:
	...
	return Vector2i(
		floori(world_pos.x / Cell.CELL_SIZE / Chunk.CHUNK_SIZE),
		floori(world_pos.y / Cell.CELL_SIZE / Chunk.CHUNK_SIZE),
	)		
...
func world_to_local(world_pos: Vector2) -> Vector2i:
	...
	var cell_x: int = floori(world_pos.x / Cell.CELL_SIZE)
	var cell_y: int = floori(world_pos.y / Cell.CELL_SIZE)
	
	var local_x = posmod(cell_x, Chunk.CHUNK_SIZE)
	var local_y = posmod(cell_y, Chunk.CHUNK_SIZE)
...
func get_cell(world_pos: Vector2i) -> Cell:
	...
	var chunk_pos: Vector2i = world_to_chunk(world_pos) # this may be incorrect
...
func set_cell_terrain(world_pos: Vector2i, terrain_set: int, terrain: int) -> void:
	...
	var chunk_pos: Vector2i = tile_to_chunk(world_pos) # this may be incorrect
	var local_pos: Vector2i = world_to_local(world_pos)
